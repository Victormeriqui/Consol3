...
namespace Display
{
    namespace Windows
    {
        ...
        private:
            ...
            void SetConsoleScreenBufferInfo(HANDLE consolescreenbuffer, short width, short height, short font_width, short font_height, const uint32_t palette[]);

            void StoreOriginalFontInfo();
            void SetFontInfo(std::wstring name, COORD font_size, short weight, short family);

        public:
            WindowsTerminalManager(short width, short height, std::wstring font_name, short font_width, short font_height, const uint32_t palette[] = palette_default);
