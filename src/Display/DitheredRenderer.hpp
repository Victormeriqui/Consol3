...
namespace Display
{
	...
	static const CHAR_INFO dithered_white[16] = {
		{ (WCHAR)32, 0x00 },  { (WCHAR)176, 0x08 }, { (WCHAR)177, 0x08 }, { (WCHAR)177, 0x80 }, { (WCHAR)176, 0x80 }, { (WCHAR)32, 0x80 },
		{ (WCHAR)176, 0x87 }, { (WCHAR)177, 0x87 }, { (WCHAR)177, 0x78 }, { (WCHAR)176, 0x78 }, { (WCHAR)32, 0x78 },  { (WCHAR)176, 0x7F },
		{ (WCHAR)177, 0x7F }, { (WCHAR)177, 0xF7 }, { (WCHAR)176, 0xF7 }, { (WCHAR)32, 0xF7 },
	};

	static const CHAR_INFO dithered_black = { (WCHAR)32, 0x00 };

	// excludes white and black (not determined by hue), and red (wraps over)
	static const std::vector<DitheredColor> sequential_dithered_colors = {
		dithered_green, dithered_blue, dithered_yellow, dithered_cyan, dithered_magenta
	};
