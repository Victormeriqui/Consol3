...
namespace Game
{
    ...
    StaticModel ModelGenerator::GeneratePlane(uint32_t tile_amount_x, uint32_t tile_amount_y, float noise_amount)
    {
        ...
        for (uint32_t y = 0; y < (tile_amount_y + 1); y++)
        {
            for (uint32_t x = 0; x < (tile_amount_x + 1); x++)
            {
                ...
                Vector2 texture_coordinates(Util::Lerp((float)x, 0, (float)(tile_amount_x + 1), 0.0f, 1.0f), Util::Lerp((float)y, 0, (float)(tile_amount_y + 1), 0.0f, 1.0f));

                vertices.push_back(Vertex(Vector3(x * tile_width, noise * (0.01f * noise_amount), y * tile_height), plane_normal, texture_coordinates));
    ...
    std::optional<SphereEdge> ModelGenerator::GetExistingSphereEdge(const std::vector<SphereEdge>& edges, uint32_t vert0_idx, uint32_t vert1_idx) const
    {
    ...
    uint32_t ModelGenerator::GetOrAddSubdividedVertex(std::vector<Vertex>& vertices, std::vector<SphereEdge>& edges, uint32_t vert0_idx, uint32_t vert1_idx)
    {
