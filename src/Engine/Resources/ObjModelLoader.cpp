...
namespace Engine
{
	namespace Resources
	{
		...
		bool ObjModelLoader::LoadModel(const std::string& filename,
									   ...
									   NormalGenerationOptions options)
		{
			...
			out_vertices.reserve(CountElements(file_stream, std::string("v ")));
			out_indices.reserve(CountElements(file_stream, std::string("f ")) * 3);
			...
			case NormalGenerationOptions::GENERATE_FORCED:
				CalculateNormals(out_vertices, out_indices);
				break;
