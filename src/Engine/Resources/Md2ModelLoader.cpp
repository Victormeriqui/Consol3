...
namespace Engine
{
    namespace Resources
    {
        ...
        bool Md2ModelLoader::LoadStaticModel(const std::string& filename, std::vector<Vertex>& out_vertices, std::vector<uint32_t>& out_indices, ModelLoadingOptions options)
        {
            // TODO load only one frame's vertices
            return false;
        }

        bool Md2ModelLoader::LoadAnimatedModel(const std::string& filename, std::vector<Frame>& out_frames, std::vector<uint32_t>& out_indices, std::map<std::string, Animation>& out_animations, ModelLoadingOptions options)
        {
            ...
            */
            ...
            for (uint16_t i = 0; i < header.frames_count; i++)
            {
                ...
                for (uint32_t j = 0; j < header.vertices_count; j++)
                {
                    ...
                    Vertex vertex = Vertex(Vector3(((float)pos_x * scale.x) + translation.x, ((float)pos_y * scale.y) + translation.y, ((float)pos_z * scale.z) + translation.z), texturecoord);
